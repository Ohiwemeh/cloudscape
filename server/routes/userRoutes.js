  ...
  try {
    ...
    if (!admin) {
      await User.create({
        name: 'Admin',
        email: adminEmail,
        password: 'cloudadmin1G23!', // Make sure this matches your intended password
        isAdmin: true
      });
...
router.post("/register", async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      name: user.name,
      email: user.email,
      isAdmin: user.isAdmin, // Include isAdmin here too
      token,
    });
  } catch (err) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (user && (await user.matchPassword(password))) {
      ...
      // This is the one and only response
      res.json(userData);
      ...
      // The duplicate res.json() has been REMOVED
    } else {
...
router.get("/me", protect, async (req, res) => {
  ...
  // Send the full user object from req.user
  res.json({
    _id: req.user._id,
    name: req.user.name,
...
router.put("/profile", protect, async (req, res) => {
  try {
    ...
    sendEmail({
      ...
      html: `<p>Hi ${updatedUser.name},</p><p>Your account details were just updated. If this wasn't you, please secure your account immediately.</p>`,
    });
